...
per_lib<-function(lib, otu, map){
  ...
  ##THE PLOTTTT
  p<- ggplot(data = Fwd_list[["data_to_plot"]], aes(x=Rev, y=Fwd, fill=N_reads))  
  ...
  q<- ggplot(data = Fwd_list[["data_to_plot"]], aes(x=Rev, y=Fwd, fill=N_DUPs))  
  q<-q+ geom_raster()+scale_fill_distiller(palette = "Spectral" , name="# unique seqs")+theme_bw()+ggtitle(paste0("Library ",lib))+
   theme(plot.title = element_text(hjust = 0.5),
        axis.text.y = element_text(face="bold",colour = Fwd_list[["color_list"]]),
        axis.text.x = element_text(face="bold",colour = x_color, angle = 90))
  
  #  p+geom_tile(aes(fill=cut_interval(V3,5)))+scale_fill_brewer(palette = "YlGnBu")
####### Generate a second plot only with Green - expected - barcodes########
  ...
  return(list(plot_abundance=p, plot_uniques=q,missing=a,unexpectedF=Fwd_list[["Unexpected_barcodes"]], unexpectedR=Rev_list[["Unexpected_barcodes"]],
              data_to_plot=Fwd_list[["data_to_plot"]]))
}